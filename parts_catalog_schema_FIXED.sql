-- Parts Catalog Schema for Xpose Management (FIXED for TEXT job IDs)
...
-- FIXED: job_id is now TEXT to match your jobs in JSONB
...
-- Catalog Vehicles - Everyone can read, authenticated users can add
CREATE POLICY "catalog_vehicles_read" ON catalog_vehicles
  FOR SELECT USING (true);

CREATE POLICY "catalog_vehicles_insert" ON catalog_vehicles
...
-- Job Parts - Users can only see/modify parts for their shop
...
SELECT 'Parts catalog schema created successfully! ✅' as message,
       'Tables created: catalog_categories, catalog_vehicles, catalog_parts, job_parts' as info;
